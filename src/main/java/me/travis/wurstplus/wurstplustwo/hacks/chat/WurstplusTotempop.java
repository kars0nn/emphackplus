...
public class WurstplusTotempop extends WurstplusHack {
    ...
    public WurstplusTotempop() {
		...
		this.name        = "CumCounter";
		this.tag         = "TotemPopCounter";
		this.description = "dude idk";
    }
        ...
        if (event.get_packet() instanceof SPacketEntityStatus) {
            ...
            if (packet.getOpCode() == 35) {
                ...
                if (WurstplusFriendUtil.isFriend(entity.getName())) {
                    WurstplusMessageUtil.send_client_message( red + "" + bold + " CumCount " + reset + grey + " > " + reset + "bro, " + bold + green + entity.getName() + reset + " has came " + bold + count + reset + " times! you should go help them!");
                } else {
                    WurstplusMessageUtil.send_client_message( red + "" + bold + " CumCount " + reset + grey + " > " + reset + "bro, " + bold + red + entity.getName() + reset + " has came " + bold + count + reset + " times. what a fucking loser");
                }
        ...
        for (EntityPlayer player : mc.world.playerEntities) {
            ...
            if (player.isDead || player.getHealth() <= 0) {
                ...
                if (WurstplusFriendUtil.isFriend(player.getName())) {
                    WurstplusMessageUtil.send_client_message( red + "" + bold + " CumCount " + reset + grey + " > " + reset + "bro, " + bold + green + player.getName() + reset + " just fucking DIED after cumming " + bold + count + reset + " times. RIP :pray:");
                } else {
                    WurstplusMessageUtil.send_client_message( red + "" + bold + " CumCount " + reset + grey + " > " + reset + "bro, " + bold + red + player.getName() + reset + " just fucking DIED after cumming " + bold + count + reset + " times... fucking pussy");
                }
